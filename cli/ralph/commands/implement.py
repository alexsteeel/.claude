...
def run_claude_fix(
    ...
    console.print(f"[cyan]Claude fix iteration {iteration}{' (resuming session)' if resume_session else ''}...[/cyan]")
    ...
    if result.error_type.is_success:
        console.print(f"[green]Claude fix done ({format_duration(result.duration_seconds)}, ${result.cost_usd:.2f})[/green]")
        return True
