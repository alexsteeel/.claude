...
from ralph.commands.implement import (
...
class TestRunCodexReview:
    ...
    @patch("ralph.commands.implement.shutil.which", return_value=None)
...
class TestRunCodexReviewLoop:
    ...
    @patch("ralph.commands.implement.run_codex_review")
    def test_lgtm_first_iteration(self, mock_review, temp_dir, settings, session_log):
        mock_review.return_value = (True, True)

        result = run_codex_review_loop(
            "proj#1", temp_dir, temp_dir, settings, session_log
        )
        assert result is True
        assert mock_review.call_count == 1
        session_log.append.assert_any_call("Codex LGTM after 1 iteration(s)")

    @patch("ralph.commands.implement._create_fixup_commit")
    @patch("ralph.commands.implement.run_claude_fix")
    @patch("ralph.commands.implement.run_codex_review")
    def test_lgtm_second_iteration(self, mock_review, mock_fix, mock_fixup, temp_dir, settings, session_log):
    ...
    @patch("ralph.commands.implement.run_claude_fix")
    @patch("ralph.commands.implement.run_codex_review")
    def test_max_iterations_reached(self, mock_review, mock_fix, mock_fixup, temp_dir, settings, session_log):
        mock_review.return_value = (True, False)
        mock_fix.return_value = True
        ...
        session_log.append.assert_any_call(
            "Codex review: max iterations (3) reached without LGTM"
        )
    ...
    def test_no_fixup_on_first_iteration_lgtm(self, mock_review, temp_dir, settings, session_log):
        ...
        with patch("ralph.commands.implement._create_fixup_commit") as mock_fixup:
            run_codex_review_loop(
                "proj#1", temp_dir, temp_dir, settings, session_log
            )
    ...
    @patch("ralph.commands.implement.run_claude_fix")
    @patch("ralph.commands.implement.run_codex_review")
    def test_session_id_passed_to_fix(self, mock_review, mock_fix, mock_fixup, temp_dir, settings, session_log):
    ...
    @patch("ralph.commands.implement.run_claude_fix")
    @patch("ralph.commands.implement.run_codex_review")
    def test_no_session_id_works(self, mock_review, mock_fix, mock_fixup, temp_dir, settings, session_log):
...
class TestRunClaudeFix:
    ...
    @patch("ralph.commands.implement.run_claude")
    def test_passes_resume_session(self, mock_run_claude, temp_dir, settings):
        ...
        run_claude_fix(
            "proj#1", temp_dir, temp_dir / "fix.log", 1, settings,
...
class TestCreateFixupCommit:
    ...
    def test_creates_fixup_when_changes(self, mock_run, temp_dir, session_log):
        ...
        session_log.append.assert_called_once_with(
            "Created fixup commit for codex review fixes"
        )
